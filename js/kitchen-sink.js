/*
 * 重写系统返回键以兼容Framework7的侧栏关闭、view后退
 */
function plusReady(){
	//处理view的后退与webview后退
	var first = null;
	plus.key.addEventListener('backbutton',function(){
		...
		if($$('body').hasClass('with-panel-left-cover')||$$('body').hasClass('with-panel-right-reveal')){//如果侧栏打开则关闭侧栏。这里只用到了这两种侧栏（侧栏一共有四中效果：left-cover，left-reveal，right-cover，right-reveal）
			myApp.closePanel();
		}else{
			if (mainView.history.length>1) { //如果view可以后退，则执行view的后退
...
/*
 * 以上是重写系统返回键以兼容Framework7的侧栏关闭、view后退
 */
